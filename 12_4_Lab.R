###Local Regression, LDA, Splines Lab
...
#Boxplots
ggplot(data=diamonds, mapping=aes(color=cut_number(price,10),y=carat))+geom_boxplot()+coord_flip()+xlab("Price") #flip coordinates to use less space

##Lab Example on Loess
help(loess) #Fit polynomial surface determined by one or more numerical predictors, using local fitting
data(cars)
cars.lo <- loess(dist ~ speed, cars)
predict(cars.lo, data.frame(speed = seq(5,30,1)), se = TRUE)

#Allow Extrapolation (fill in missing data)
...
##Visualize Cars data
require(graphics)
help(lowess) #This function performs the computations for the LOWESS smoother which uses locally-weighted polynomial regression
plot(cars, main = "lowess(cars)")
lines(lowess(cars), col = 2) #lowess plots local regression line
lines(lowess(cars, f = .2), col = 3)
legend(5, 120, c(paste("f = ", c("2/3", ".2"))), lty = 1, col = 2:3)


require(graphics)
help(supsmu) #another smoothing technique for the regression line
with(cars, {
  plot(speed, dist)
